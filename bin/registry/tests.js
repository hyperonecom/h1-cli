...
ava.serial('registry manage repositories & tags', async t => {
    ...
    try {
        ...
        t.true(repositories.some(x => x.id === hubImage));
        // const tag = await tests.run(`registry repository tag show --registry ${registry.name} --image ${hubImage} --tag ${tagName}`);
...
ava.serial('registry reachable through custom domain', async t => {
    ...
    try {
        ...
        await tests.run(`registry update domain --registry ${registry.id} --domain ${host}`);
