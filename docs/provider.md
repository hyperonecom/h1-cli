## h1 provider

```
h1 provider

Synopsis

  $ h1 provider <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  agent   Management of agent resource
```

### h1 provider agent

```
h1 provider agent

Synopsis

  $ h1 provider agent <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  create            Create provider/agent [Operation ID: provider_project_agent_create]     
  list              List provider/agent [Operation ID: provider_project_agent_list]         
  show              Get provider/agent [Operation ID: provider_project_agent_get]           
  update            Update provider/agent [Operation ID: provider_project_agent_update]     
  delete            Delete provider/agent [Operation ID: provider_project_agent_delete]     
  start             Start provider/agent [Operation ID: provider_project_agent_start]       
  suspend           Suspend provider/agent [Operation ID: provider_project_agent_suspend]   
  inspect           Inspect provider/agent [Operation ID: provider_project_agent_inspect]   
  transfer          Transfer provider/agent [Operation ID: provider_project_agent_transfer] 
  credential        Manage credentials of the agent                                         
  enabled-service   Manage enabledServices of the agent                                     
  resource          Manage resources of the agent                                           
  metric            Manage metrics of the agent                                             
  service           Manage services of the agent                                            
  tag               Manage tags of the agent                                                
  event             Manage events of the agent
```

#### h1 provider agent create

```
h1 provider agent create

  Create provider/agent [Operation ID: provider_project_agent_create] 

Synopsis

  $ h1 provider agent create <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent create --project 5f64e2468c71177993874510 --name simple-  
  agent --service                                                               
  /billing/project/5f64e2468c71177993874510/service/5cb9e2c1467ad031486f9d87    

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri          Project Id                                          
  --location id-or-uri         Location Id. Default value is pl-waw-1              
  --x-idempotency-key string   Idempotency key                                     
  --name string                Agent name                                          
  --service id-or-uri          Agent service. Provide ID or URI of billing/service 
  --tag key=key,value=value    Tag collection                                      
  --skeleton
```

#### h1 provider agent list

```
h1 provider agent list

  List provider/agent [Operation ID: provider_project_agent_list] 

Synopsis

  $ h1 provider agent list <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent list --project 5f64e2468c71177993874510                   

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri         Project Id                             
  --location id-or-uri        Location Id. Default value is pl-waw-1 
  --name string               Filter by name                         
  --enabled-services string   Filter by enabledServices              
  --tag.value string          Filter by tag.value                    
  --tag.key string            Filter by tag.key                      
  --skeleton
```

#### h1 provider agent show

```
h1 provider agent show

  Get provider/agent [Operation ID: provider_project_agent_get] 

Synopsis

  $ h1 provider agent show <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent show --project 5f64e2468c71177993874510 --agent           
  5f577a24494c5cfdec7830e5                                                      

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --skeleton
```

#### h1 provider agent update

```
h1 provider agent update

  Update provider/agent [Operation ID: provider_project_agent_update] 

Synopsis

  $ h1 provider agent update <options> 

Example

                                                                                
  Rename                                                                        
                                                                                
  $ h1 provider agent update --project 5f64e2468c71177993874510 --agent         
  5f577a24494c5cfdec7830e5 --name agent-renamed                                 

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri    Project Id                                                  
  --location id-or-uri   Location Id. Default value is pl-waw-1                      
  --agent id-or-uri      Agent Id                                                    
  --name string          Agent name. Requires permissions provider/agent.name/update 
  --skeleton
```

#### h1 provider agent delete

```
h1 provider agent delete

  Delete provider/agent [Operation ID: provider_project_agent_delete] 

Synopsis

  $ h1 provider agent delete <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent delete --project 5f64e2468c71177993874510 --agent         
  5f577a24494c5cfdec7830e5                                                      

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --skeleton
```

#### h1 provider agent start

```
h1 provider agent start

  Start provider/agent [Operation ID: provider_project_agent_start] 

Synopsis

  $ h1 provider agent start <options> 

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri          Project Id                             
  --location id-or-uri         Location Id. Default value is pl-waw-1 
  --agent id-or-uri            Agent Id                               
  --x-idempotency-key string   Idempotency key                        
  --skeleton
```

#### h1 provider agent suspend

```
h1 provider agent suspend

  Suspend provider/agent [Operation ID: provider_project_agent_suspend] 

Synopsis

  $ h1 provider agent suspend <options> 

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri          Project Id                             
  --location id-or-uri         Location Id. Default value is pl-waw-1 
  --agent id-or-uri            Agent Id                               
  --x-idempotency-key string   Idempotency key                        
  --skeleton
```

#### h1 provider agent inspect

```
h1 provider agent inspect

  Inspect provider/agent [Operation ID: provider_project_agent_inspect] 

Synopsis

  $ h1 provider agent inspect <options> 

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri          Project Id                             
  --location id-or-uri         Location Id. Default value is pl-waw-1 
  --agent id-or-uri            Agent Id                               
  --x-idempotency-key string   Idempotency key                        
  --skeleton
```

#### h1 provider agent transfer

```
h1 provider agent transfer

  Transfer provider/agent [Operation ID: provider_project_agent_transfer] 

Synopsis

  $ h1 provider agent transfer <options> 

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri          Project Id                                                                    
  --location id-or-uri         Location Id. Default value is pl-waw-1                                        
  --agent id-or-uri            Agent Id                                                                      
  --x-idempotency-key string   Idempotency key                                                               
  --project id-or-uri          Agent project. Provide ID or URI of iam/project. Requires permissions         
                               provider/agent/create                                                         
  --skeleton
```

#### h1 provider agent credential

```
h1 provider agent credential

Synopsis

  $ h1 provider agent credential <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  create   Create provider/agent.credential [Operation ID: provider_project_agent_credential_create] 
  list     List provider/agent.credential [Operation ID: provider_project_agent_credential_list]     
  show     Get provider/agent.credential [Operation ID: provider_project_agent_credential_get]       
  update   Update provider/agent.credential [Operation ID: provider_project_agent_credential_patch]  
  delete   Delete provider/agent.credential [Operation ID: provider_project_agent_credential_delete]
```

##### h1 provider agent credential create

```
h1 provider agent credential create

  Create provider/agent.credential [Operation ID:                               
  provider_project_agent_credential_create]                                     

Synopsis

  $ h1 provider agent credential create <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent credential create --project 5f64e2468c71177993874510      
  --agent 5f577a24494c5cfdec7830e5 --name agent-credential --type ssh --value   
  ssh-rsa                                                                       
   

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --name string          Credential name                        
  --type ssh             Credential type                        
  --value string         Credential value                       
  --token string         Credential token                       
  --skeleton
```

##### h1 provider agent credential list

```
h1 provider agent credential list

  List provider/agent.credential [Operation ID:                                 
  provider_project_agent_credential_list]                                       

Synopsis

  $ h1 provider agent credential list <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent credential list --project 5f64e2468c71177993874510        
  --agent 5f577a24494c5cfdec7830e5                                              

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --skeleton
```

##### h1 provider agent credential show

```
h1 provider agent credential show

  Get provider/agent.credential [Operation ID:                                  
  provider_project_agent_credential_get]                                        

Synopsis

  $ h1 provider agent credential show <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent credential show --project 5f64e2468c71177993874510        
  --agent 5f577a24494c5cfdec7830e5 --credential 5f60cbbe494c5cfdec81cc6b        

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri      Project Id                             
  --location id-or-uri     Location Id. Default value is pl-waw-1 
  --agent id-or-uri        Agent Id                               
  --credential id-or-uri   credentialId                           
  --skeleton
```

##### h1 provider agent credential update

```
h1 provider agent credential update

  Update provider/agent.credential [Operation ID:                               
  provider_project_agent_credential_patch]                                      

Synopsis

  $ h1 provider agent credential update <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent credential update --project 5f64e2468c71177993874510      
  --agent 5f577a24494c5cfdec7830e5 --credential 5f60cbbe494c5cfdec81cc6b --name 
  renamed-credential-agent                                                      

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri      Project Id                                                                  
  --location id-or-uri     Location Id. Default value is pl-waw-1                                      
  --agent id-or-uri        Agent Id                                                                    
  --credential id-or-uri   credentialId                                                                
  --name string            Credential name. Requires permissions provider/agent.credential.name/update 
  --skeleton
```

##### h1 provider agent credential delete

```
h1 provider agent credential delete

  Delete provider/agent.credential [Operation ID:                               
  provider_project_agent_credential_delete]                                     

Synopsis

  $ h1 provider agent credential delete <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent credential delete --project 5f64e2468c71177993874510      
  --agent 5f577a24494c5cfdec7830e5 --credential 5f60cbbe494c5cfdec81cc6b        

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri      Project Id                             
  --location id-or-uri     Location Id. Default value is pl-waw-1 
  --agent id-or-uri        Agent Id                               
  --credential id-or-uri   credentialId                           
  --skeleton
```

#### h1 provider agent enabled-service

```
h1 provider agent enabled-service

Synopsis

  $ h1 provider agent enabled-service <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  create   Create provider/agent.enabledService [Operation ID: provider_project_agent_enabledService_create] 
  list     List provider/agent.enabledService [Operation ID: provider_project_agent_enabledService_list]     
  show     Get provider/agent.enabledService [Operation ID: provider_project_agent_enabledService_get]       
  delete   Delete provider/agent.enabledService [Operation ID: provider_project_agent_enabledService_delete]
```

##### h1 provider agent enabled-service create

```
h1 provider agent enabled-service create

  Create provider/agent.enabledService [Operation ID:                           
  provider_project_agent_enabledService_create]                                 

Synopsis

  $ h1 provider agent enabled-service create <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent enabled-service create --project 5f64e2468c71177993874510 
  --agent 5f577a24494c5cfdec7830e5 --service                                    
  /billing/project/platform/service/5cb9e2c8467ad031486f9d88                    

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                                                   
  --location id-or-uri   Location Id. Default value is pl-waw-1                       
  --agent id-or-uri      Agent Id                                                     
  --service id-or-uri    EnabledService service. Provide ID or URI of billing/service 
  --skeleton
```

##### h1 provider agent enabled-service list

```
h1 provider agent enabled-service list

  List provider/agent.enabledService [Operation ID:                             
  provider_project_agent_enabledService_list]                                   

Synopsis

  $ h1 provider agent enabled-service list <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent enabled-service list --project 5f64e2468c71177993874510   
  --agent 5f577a24494c5cfdec7830e5                                              

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --skeleton
```

##### h1 provider agent enabled-service show

```
h1 provider agent enabled-service show

  Get provider/agent.enabledService [Operation ID:                              
  provider_project_agent_enabledService_get]                                    

Synopsis

  $ h1 provider agent enabled-service show <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent enabled-service show --project 5f64e2468c71177993874510   
  --agent 5f577a24494c5cfdec7830e5 --enabled-service 5c9cc2d0255c16c3e899a4ea   

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri           Project Id                             
  --location id-or-uri          Location Id. Default value is pl-waw-1 
  --agent id-or-uri             Agent Id                               
  --enabled-service id-or-uri   enabledServiceId                       
  --skeleton
```

##### h1 provider agent enabled-service delete

```
h1 provider agent enabled-service delete

  Delete provider/agent.enabledService [Operation ID:                           
  provider_project_agent_enabledService_delete]                                 

Synopsis

  $ h1 provider agent enabled-service delete <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent enabled-service delete --project 5f64e2468c71177993874510 
  --agent 5f577a24494c5cfdec7830e5 --enabled-service 5c9cc2d0255c16c3e899a4ea   

Global options

  --help                                      Show help message and exit.                                                   
  --verbose                                   Make the operation more talkative.                                            
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                       
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state,    
                                              flavour:flavour}                                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.            
  --as uri                                    Act as another actor eg. service account                                      
  --no-wait                                   In case of queued event do not wait for completion                            

Operation options

  --project id-or-uri           Project Id                             
  --location id-or-uri          Location Id. Default value is pl-waw-1 
  --agent id-or-uri             Agent Id                               
  --enabled-service id-or-uri   enabledServiceId                       
  --skeleton
```

#### h1 provider agent resource

```
h1 provider agent resource

Synopsis

  $ h1 provider agent resource <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  list       List provider/agent.resource [Operation ID: provider_project_agent_resource_list]         
  show       Get provider/agent.resource [Operation ID: provider_project_agent_resource_get]           
  recreate   Recreate provider/agent.resource [Operation ID: provider_project_agent_resource_recreate] 
  inspect    Inspect provider/agent.resource [Operation ID: provider_project_agent_resource_inspect]   
  event      Manage events of the resource
```

##### h1 provider agent resource list

```
h1 provider agent resource list

  List provider/agent.resource [Operation ID:                                   
  provider_project_agent_resource_list]                                         

Synopsis

  $ h1 provider agent resource list <options> 

Global options

  --help                                      Show help message and exit.                                                 
  --verbose                                   Make the operation more talkative.                                          
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                     
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state}  
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.          
  --as uri                                    Act as another actor eg. service account                                    
  --no-wait                                   In case of queued event do not wait for completion                          

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --skeleton
```

##### h1 provider agent resource show

```
h1 provider agent resource show

  Get provider/agent.resource [Operation ID: provider_project_agent_resource_get] 

Synopsis

  $ h1 provider agent resource show <options> 

Global options

  --help                                      Show help message and exit.                                                 
  --verbose                                   Make the operation more talkative.                                          
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                     
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state}  
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.          
  --as uri                                    Act as another actor eg. service account                                    
  --no-wait                                   In case of queued event do not wait for completion                          

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --resource id-or-uri   resourceId                             
  --skeleton
```

##### h1 provider agent resource recreate

```
h1 provider agent resource recreate

  Recreate provider/agent.resource [Operation ID:                               
  provider_project_agent_resource_recreate]                                     

Synopsis

  $ h1 provider agent resource recreate <options> 

Global options

  --help                                      Show help message and exit.                                                 
  --verbose                                   Make the operation more talkative.                                          
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                     
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state}  
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.          
  --as uri                                    Act as another actor eg. service account                                    
  --no-wait                                   In case of queued event do not wait for completion                          

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --resource id-or-uri   resourceId                             
  --skeleton
```

##### h1 provider agent resource inspect

```
h1 provider agent resource inspect

  Inspect provider/agent.resource [Operation ID:                                
  provider_project_agent_resource_inspect]                                      

Synopsis

  $ h1 provider agent resource inspect <options> 

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{value:@}               
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --resource id-or-uri   resourceId                             
  --skeleton
```

##### h1 provider agent resource event

```
h1 provider agent resource event

Synopsis

  $ h1 provider agent resource event <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  list   List provider/agent.event [Operation ID: provider_project_agent_resource_event_list]
```

###### h1 provider agent resource event list

```
h1 provider agent resource event list

  List provider/agent.event [Operation ID:                                      
  provider_project_agent_resource_event_list]                                   

Synopsis

  $ h1 provider agent resource event list <options> 

Global options

  --help                                      Show help message and exit.                                                 
  --verbose                                   Make the operation more talkative.                                          
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                     
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state}  
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.          
  --as uri                                    Act as another actor eg. service account                                    
  --no-wait                                   In case of queued event do not wait for completion                          

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --resource id-or-uri   resourceId                             
  --$limit string        $limit                                 
  --$skip string         $skip                                  
  --skeleton
```

#### h1 provider agent metric

```
h1 provider agent metric

Synopsis

  $ h1 provider agent metric <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  list    List provider/agent.metric [Operation ID: provider_project_agent_metric_list] 
  show    Get provider/agent.metric [Operation ID: provider_project_agent_metric_get]   
  point   Manage points of the metric
```

##### h1 provider agent metric list

```
h1 provider agent metric list

  List provider/agent.metric [Operation ID: provider_project_agent_metric_list] 

Synopsis

  $ h1 provider agent metric list <options> 

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --skeleton
```

##### h1 provider agent metric show

```
h1 provider agent metric show

  Get provider/agent.metric [Operation ID: provider_project_agent_metric_get] 

Synopsis

  $ h1 provider agent metric show <options> 

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --metric id-or-uri     metricId                               
  --skeleton
```

##### h1 provider agent metric point

```
h1 provider agent metric point

Synopsis

  $ h1 provider agent metric point <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  list   List provider/agent.point [Operation ID: provider_project_agent_metric_point_list]
```

###### h1 provider agent metric point list

```
h1 provider agent metric point list

  List provider/agent.point [Operation ID:                                      
  provider_project_agent_metric_point_list]                                     

Synopsis

  $ h1 provider agent metric point list <options> 

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{value:@}               
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --metric id-or-uri     metricId                               
  --interval string      interval                               
  --timespan string      timespan                               
  --skeleton
```

#### h1 provider agent service

```
h1 provider agent service

Synopsis

  $ h1 provider agent service <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  list   List provider/agent.service [Operation ID: provider_project_agent_service_list] 
  show   Get provider/agent.service [Operation ID: provider_project_agent_service_get]
```

##### h1 provider agent service list

```
h1 provider agent service list

  List provider/agent.service [Operation ID: provider_project_agent_service_list] 

Synopsis

  $ h1 provider agent service list <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent service list --project 5f64e2468c71177993874510 --agent   
  5f577a24494c5cfdec7830e5                                                      

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --skeleton
```

##### h1 provider agent service show

```
h1 provider agent service show

  Get provider/agent.service [Operation ID: provider_project_agent_service_get] 

Synopsis

  $ h1 provider agent service show <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent service show --project 5f64e2468c71177993874510 --agent   
  5f577a24494c5cfdec7830e5 --service 5f60cbbe494c5cfdec81cc6b                   

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id, name:name}      
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --service id-or-uri    serviceId                              
  --skeleton
```

#### h1 provider agent tag

```
h1 provider agent tag

Synopsis

  $ h1 provider agent tag <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  create   Create provider/agent.tag [Operation ID: provider_project_agent_tag_create] 
  list     List provider/agent.tag [Operation ID: provider_project_agent_tag_list]     
  show     Get provider/agent.tag [Operation ID: provider_project_agent_tag_get]       
  delete   Delete provider/agent.tag [Operation ID: provider_project_agent_tag_delete]
```

##### h1 provider agent tag create

```
h1 provider agent tag create

  Create provider/agent.tag [Operation ID: provider_project_agent_tag_create] 

Synopsis

  $ h1 provider agent tag create <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent tag create --project 5f64e2468c71177993874510 --agent     
  5f577a24494c5cfdec7830e5 --key x --value x                                    

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id}                 
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --key string           Tag key                                
  --value string         Tag value                              
  --skeleton
```

##### h1 provider agent tag list

```
h1 provider agent tag list

  List provider/agent.tag [Operation ID: provider_project_agent_tag_list] 

Synopsis

  $ h1 provider agent tag list <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent tag list --project 5f64e2468c71177993874510 --agent       
  5f577a24494c5cfdec7830e5                                                      

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id}                 
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --skeleton
```

##### h1 provider agent tag show

```
h1 provider agent tag show

  Get provider/agent.tag [Operation ID: provider_project_agent_tag_get] 

Synopsis

  $ h1 provider agent tag show <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent tag show --project 5f64e2468c71177993874510 --agent       
  5f577a24494c5cfdec7830e5 --tag 5f60cbbe494c5cfdec81cc6b                       

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string. Default value is [].{id:id}                 
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --tag id-or-uri        tagId                                  
  --skeleton
```

##### h1 provider agent tag delete

```
h1 provider agent tag delete

  Delete provider/agent.tag [Operation ID: provider_project_agent_tag_delete] 

Synopsis

  $ h1 provider agent tag delete <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent tag delete --project 5f64e2468c71177993874510 --agent     
  5f577a24494c5cfdec7830e5 --tag 5f60cbbe494c5cfdec81cc6b                       

Global options

  --help                                      Show help message and exit.                                        
  --verbose                                   Make the operation more talkative.                                 
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml            
  --query string                              JMESPath query string                                              
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available. 
  --as uri                                    Act as another actor eg. service account                           
  --no-wait                                   In case of queued event do not wait for completion                 

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --tag id-or-uri        tagId                                  
  --skeleton
```

#### h1 provider agent event

```
h1 provider agent event

Synopsis

  $ h1 provider agent event <group> <command> 

Options

  --help    Show help message and exit. 

Command List

  list   List provider/agent.event [Operation ID: provider_project_agent_event_list] 
  show   Get provider/agent.event [Operation ID: provider_project_agent_event_get]
```

##### h1 provider agent event list

```
h1 provider agent event list

  List provider/agent.event [Operation ID: provider_project_agent_event_list] 

Synopsis

  $ h1 provider agent event list <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent event list --project 5f64e2468c71177993874510 --agent     
  5f577a24494c5cfdec7830e5                                                      

Global options

  --help                                      Show help message and exit.                                                 
  --verbose                                   Make the operation more talkative.                                          
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                     
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state}  
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.          
  --as uri                                    Act as another actor eg. service account                                    
  --no-wait                                   In case of queued event do not wait for completion                          

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --$limit string        $limit                                 
  --$skip string         $skip                                  
  --skeleton
```

##### h1 provider agent event show

```
h1 provider agent event show

  Get provider/agent.event [Operation ID: provider_project_agent_event_get] 

Synopsis

  $ h1 provider agent event show <options> 

Example

                                                                                
  Simple                                                                        
                                                                                
  $ h1 provider agent event show --project 5f64e2468c71177993874510 --agent     
  5f577a24494c5cfdec7830e5 --event 5f60cbbe494c5cfdec81cc6b                     

Global options

  --help                                      Show help message and exit.                                                 
  --verbose                                   Make the operation more talkative.                                          
  -o, --output tsv,list,json,js,id,uri,yaml   Specify output format of command. Default value is yaml                     
  --query string                              JMESPath query string. Default value is [].{id:id, name:name, state:state}  
  --passport-file path                        Passport file. Default value is ~/.h1/passport.json, if available.          
  --as uri                                    Act as another actor eg. service account                                    
  --no-wait                                   In case of queued event do not wait for completion                          

Operation options

  --project id-or-uri    Project Id                             
  --location id-or-uri   Location Id. Default value is pl-waw-1 
  --agent id-or-uri      Agent Id                               
  --event id-or-uri      eventId                                
  --skeleton
```

