...
exports.examples = (command, dirname, context = { epilog: 'default' }) => {
    ...
    try {
        ...
        // structured-cli use argparse which use sprintf, so escape '%' is required.
        // See https://github.com/nodeca/argparse/issues/125
        const markdown_escaped = markdown.replace(/%/g, '%%');
        command.epilog = marked(`# Examples\n${markdown_escaped}`);
        command.examples = markdown;

    } catch (err) {
        if (process.env.NODE_ENV !== 'production') {
            throw Error(`Unable to generate examples for ${Cli.get_full_name(command)}: ${err}`);
        }
    }
