...
const resourceLifeCycle = (type, options = {}) => wrap(type, options, async (t, resource) => {
    ...
    if (!options.skipService) {
        ...
        const service_list = await run(`${type} service list ${options.serviceListParams || ''} --${getOption(type)} ${resource.name}`);
        t.true(Array.isArray(service_list));
        t.true(service_list.length > 0, 'The resource should have at least one service');
        const service = await run(`${type} service show ${options.serviceShowParams || ''} --${getOption(type)} ${resource.name} --service ${service_list[0]._id}`);
