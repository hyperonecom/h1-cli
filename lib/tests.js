...
const getMyIp = async () => {
    // The reliability of external services is not important to us, so
    // we have multiple failovers
    try {
        const resp = await get('https://api.ipify.org?format=json');
        return resp.body.ip;
    } catch (err) {
        try {
            const resp = await get('http://httpbin.org/ip');
            return resp.body.origin;
        } catch (err) {
            const resp = await get('http://ipv4bot.whatismyipaddress.com/');
            return resp.text;
        }
    }
};
...
const DELAY = {
    ...
    reservation_bill: 3 * 60 * 1000,
    resource_bill: 3 * 60 * 1000,
};
