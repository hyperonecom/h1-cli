...
const wrap = (type, options = {}, func) => async t => {
    ...
    const createParams = options.skipTag ? options.createParams : `${options.createParams} --tag ON_CREATE=${now}`;
    const resource = await run(`${type} ${options.createCommand || 'create'} ${createParams} `);
    try {
...
const resourceLifeCycle = (type, options = {}) => wrap(type, options, async (t, resource) => {
    ...
    if (!options.skipTag) {
        // Add of tag
        await run(`${type} tag add --${getOption(type)} ${resource.name} --tag protected=${now}`);
        const tag_obj = await run(`${type} tag list --${getOption(type)} ${resource.name}`);
        t.true(!!tag_obj.protected);
        t.true(!!tag_obj.ON_CREATE);

        // Protection of "protected" tag
        await t.throws(() => remove(type, resource));

        // Delete of tag
        await run(`${type} tag delete --${getOption(type)} ${resource.name} --tag protected`);
        const tag_obj_new = await run(`${type} tag list --${getOption(type)} ${resource.name}`);
        t.true(!tag_obj_new.protected);
        t.true(!!tag_obj.ON_CREATE);
    }
