...
const openApiValidator = async (schemaRef, data) => {
    const schema = {$ref: `https://${process.env.API_HOSTNAME}/swagger.json${schemaRef}`};
    const validate = await openApiAjv.compileAsync(schema);
    const valid = validate(data);
    if (!valid) throw {
        msg: 'OpenAPI Schema does not match',
...
const getName = (...names) => [
    ...names.map(name => name.replace(/[^\w]/g, '-')),
    Date.now().toString(),
].join('-');
...
const resourceLifeCycle = (type, options = {}) => wrap(type, options, async (t, resource) => {
    ...
    if (type.split(' ').length === 1 || options.schemaRef) {
        await openApiValidator(options.schemaRef || `#/components/schemas/${type}`, resource_by_id);
    }
...
const resourceAccessOfUser = (type, accessParams) => async t => {
    for (const role of ['user', 'owner', 'billing']) {
        ...
        for (const action of actions) {
            ...
            await openApiValidator(`#/components/schemas/${type}.accessRights`, access);

            const list = await run(`${type} access list`);
