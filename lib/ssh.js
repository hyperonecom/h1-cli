...
const isDocker = () => fs.promises.stat('/.dockerenv').then(() => true).catch(() => false);

const withoutArpCache = async fn => {
    // Moving the IP address to a different network interface with a different physical address
    // may cause stale ARP cache entry. Then the connection cannot be properly established.
    // In the case of containers, packets are forwarded through bridge
    // on host (usually using the docker0 bridge) so the ARP table of the host
    // needs to be refreshed on the host.
    const command = 'ip -s neigh flush all';
    try {
        // Best effort flushing ARP cache
        if (await isDocker()) {
            await runProcess(`docker run --rm --network host --privileged debian:10 ${command}`);
        }
        try {
            await runProcess(command);
        } catch (err) {
            await runProcess(`sudo ${command}`, {timeout: 1000});
        }
    } catch (err) {
        console.log('Flushing ARP cache failed', err);
    }
    return fn();
};
...
const connection = (options) => new Promise((resolve, reject) => {
    ...
    withoutArpCache(() => conn.connect({ port: 22, ...options }));
});
...
const putResource = (resource, auth, path, content) => {
    console.log(new Date().toISOString(), `[resource: ${resource.id}] Put file ${path} (content length: ${content.length})`);
    return putFile(path, content, { host: resource.fqdn, username: resource.id, ...auth });
};

const execVm = (resource, auth, cmd) => execResource(resource, { username: 'guru', ...auth }, cmd);

const putVm = (resource, auth, path, content) => putResource(resource, { username: 'guru', ...auth }, path, content);
