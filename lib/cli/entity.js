...
class Category extends Command {
    ...
    async getUsage() {
        ...
        const content = [
            ...
            {
                header: 'Command List',
                content: commands.map(x => ({ name: x.name, summary: x.summary })),
            },
        ];
        return commandLineUsage(content);
    }
    async addCommand(...cmds) {
    ...
    async exec(argv = [], parentOpts = {}) {
        ...
        const commands = await this.getCommands();
        const cmd = commands.find(x => x.name == opts.command || x.aliases.includes(opts.command));
        if (!cmd) {
            if (opts.help) {
                return this.getUsage();
            }
            ...
            const result = meant(opts.command, commands.map(x => x.name));
            const msg = [
