...
const printCommand = (name, content, {state}) => new Command({
    name,
    state,
...
const makeOperationCommand = ({ name, operation, method, path, state }) => {
...
const makeResourceCommand = async (resource, ctx) => {
    ...
    for (const { name, path } of openapi.getActions(instancePathPrefix)) {
        await cmd.addCommand(await makeOperationCommand({
            ...ctx,
            name,
            method: 'post',
    ...
    for (const child of openapi.getChild(instancePathPrefix)) {
        await cmd.addCommand(await makeResourceCommand(child, {
            path: child.path,
...
const buildNamespaceCommand = async (name, spec, ctx) => {
    const cmd = new Category({
        name,
    ...
    await cmd.addCommand(printCommand('spec', spec, ctx));
    ...
    for (const resource of spec) {
        ...
        if (resource.parent) {
            for (const parent of resource.parent) {
                ...
                await parent_cmd.addCommand(await makeResourceCommand(resource, {
                    ...ctx,
                    path: `${path}/${parent}/{${parent}Id}/${resource.type}`,
                }));
            }
        } else {
            await cmd.addCommand(await makeResourceCommand(resource, {
                ...ctx,
                path: `${path}/${resource.type}`,
            }));
        }
    }
    await cmd.addCommand(...Object.values(parent_cmds));
...
const buildProgram = async (options = {}) => {
    ...
    for (const [name, spec] of Object.entries(namespaces)) {
        ...
        await program.addCommand(await buildNamespaceCommand(name, spec, {
            ...options,
            path: `/${name}`,
        }));
    }
