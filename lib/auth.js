...
exports.federate = async (token) => {
    const body = await exchange(token);
    updateToken(body);
};
...
const exchange = async (token, audience = API_URL, options = {}) => {
    const openid_configuration = await request.get(`${API_URL}/.well-known/openid-configuration`).then(resp => resp.body);
...
exports.getToken = async (audience = API_URL) => {
    ...
    if (args.as) {
        ...
        const body = await exchange(access_token, audience, {
            resource: args.as,
        });
        return body.access_token;
