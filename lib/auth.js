...
const getPassportToken = async (passportFile, audience = API_HOSTNAME) => {
...
const updateToken = (body, audience = API_HOSTNAME) => {
...
const delegate = async (token, actor, audience = API_HOSTNAME) => {
    const openid_configuration = await request.get(`${API_URL}/.well-known/openid-configuration`).then(resp => resp.body);
...
const updateAudience = async (token, audience) => {
    // TODO: Merge with "delegate" and delegat to self
    const openid_configuration = await request.get(`${API_URL}/.well-known/openid-configuration`).then(resp => resp.body);
    return request.post(openid_configuration.token_endpoint).send({
        grant_type: 'urn:ietf:params:oauth:grant-type:token-exchange',
        audience,
...
const getAccessToken = async () => {
    ...
    try {
        ...
        if (error == 'invalid_grant') {
            throw cli.error.cancelled('The used refresh token is invalid / expired / revoked. Use \'login\' again. ');
        }
        throw err;
    }
};
...
exports.getToken = async (audience) => {
    let access_token;
    ...
    if (args.as) {
        const body = await delegate(args.as, access_token);
        logger('verbose', `Used delegate to switch actor to '${args.as}'`);
        return body.access_token;
    } else if (!args['passport-file'] && audience !== API_HOSTNAME) {
        const body = await updateAudience(access_token, audience);
        return body.access_token;
    }
    return access_token;
};
