...
const getHeaders = () => {
    ...
    if (process.env.H1_TOKEN) {
        headers['x-auth-token'] = process.env.H1_TOKEN;
    }

    if (process.env.H1_PROJECT || config.get('profile.project._id')) {
        headers['x-project'] = process.env.H1_PROJECT || config.get('profile.project._id');
    }
...
exports.raw = (method, uri) => api(method, uri);
