...
exports.setProject = function(value) {
    project = value;
};
...
exports.stream = (uri, query = {}, emitter) => {
    ...
    return api('get', uri)
        ...
        .on('response', rsp => {
            ...
            lineReader.on('line', function (line) {
                counter += 1;
                try {
                    emitter.emit('jsonl', JSON.parse(line));
                } catch (err) {
                    emitter.emit('error', {
                        err: err,
                        line: line,
            ...
            rsp.on('end', () => console.log('Line Reader Counter', counter));
            rsp.on('end', () => emitter.emit('end'));
            lineReader.on('error', err => emitter.emit('error', err));
        });
};
