...
const genToken = (subject, keyid, key, audience='api.hyperone.com') => {
    ...
    return jwt.sign({}, key, {
        ...
        keyid,
        audience,
        issuer: `${API_URL}${subject}`,
        subject,
    });

};

const getToken = (audience) => genToken(
    config.get('auth.identity'),
    config.get('auth.credential.id'),
    fs.readFileSync(config.get('auth.credential.path')),
    audience
);
...
const api = (method, uri, apiUrl = API_URL) => {
    return raw(method, `${apiUrl}/${uri}`)
        .on('request', (req) => {
            if (verbose) {
                logger('verbose', `${req.req.method} ${req.url}`);
                logger('verbose', 'headers', JSON.stringify(req.header, null, 2));
            }
        })
        .on('response', (res) => {
            if (verbose) {
