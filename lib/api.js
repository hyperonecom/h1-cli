...
exports.getApiKey = function(user, body) {
    return api('post', `user/${user}/session`)
        ...
        .catch(async err => {
            if (err.status === 403 && err.response.body.code === 'CHALLENGE_TOTP_REQUEST') {
                const token = await interactive.prompt('2FA token');

                return exports.getApiKey(user, Object.assign({}, body, { challenge: { totp: token.value } }));
            }
