...
const genericOptionsGroups = {
    ...
    credentials: (options) => {
        ...
        const cert = credential_types.includes('certificate');
        if (password && cert) {
...
const sort =  entries => entries.sort((a, b) => {
    const a_p = a.createOptions && a.createOptions.priority || 50;
    const b_p = b.createOptions && b.createOptions.priority || 50;
...
module.exports = {
    ...
    flatten: (cli) => {
        ...
        while (missing.length > 0) {
            ...
            nodes.push(entry);
            if (entry.children) {
                missing.unshift(...sort(entry.children));
            }
        }
        return nodes;
    },
