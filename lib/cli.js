...
module.exports = {
    ...
    createCommand: (name, opts) => {
        ['plugins', 'params', 'options', 'context'].forEach(name => {
            if (!opts[name] && opts.resource) {
                opts[name] = opts.resource[name];
            }
        });

        if (opts.genericOptions) {
            opts.options = Object.assign({},
                ...opts.genericOptions.map(group => genericOptionsGroups[group](opts)),
                opts.options,
            );
        }

        const requiredOptions = pickBy(opts.options, o => o.required);
        opts.options = pickBy(opts.options, o => !o.required);
        opts.options = pickBy(opts.options, o => !(o.ignoringScope || []).includes(config.scope()));

        const command = Cli.createCommand(name, opts);
        command.createOptions = opts;
